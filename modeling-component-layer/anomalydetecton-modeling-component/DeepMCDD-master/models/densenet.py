...
class DenseNet_DeepMCDD(nn.Module):
    def __init__(self, latent_size=342, num_classes=10):
        super(DenseNet_DeepMCDD, self).__init__()
        ...
        self.radii = torch.zeros(num_classes).cuda()

        self.fe = DenseNet3(100, num_classes)
        self.init_weights()
    ...
    def _forward(self, x):
        out = self.fe.conv1(x)
        out = self.fe.trans1(self.fe.block1(out))
        out = self.fe.trans2(self.fe.block2(out))
        out = self.fe.block3(out)
        out = self.fe.relu(self.fe.bn1(out))
        out = F.avg_pool2d(out, 8)
        out = out.view(-1, self.fe.in_planes)
        return out
