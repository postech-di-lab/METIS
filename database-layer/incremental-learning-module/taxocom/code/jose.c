...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *) "-word-emb", argc, argv)) > 0) strcpy(word_emb, argv[i + 1]);
  if ((i = ArgPos((char *) "-context-emb", argc, argv)) > 0) strcpy(context_emb, argv[i + 1]);
  if ((i = ArgPos((char *) "-doc-emb", argc, argv)) > 0) strcpy(doc_output, argv[i + 1]);
